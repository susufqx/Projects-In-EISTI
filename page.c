...
void FIFO(in * tache, int n_tache, int n_bloc);
...
void OPT (int * tache, int n_tache, int n_bloc){
  ...
  int i, j, k;
	...
	int faute = 0;
	for(i=0;i<N_TACHE;i++){
		if(exist(Liste_Bloc,n_bloc,tache[i]) != -1){  // Cette tache est deja dans la memoire
			//Ne rien faire , laisse executer
			for(k=0;k<n_bloc;k++){
				printf(" %d ", Liste_Bloc[k]);
			}
			printf("\n");
		}else if(exist(Liste_Bloc, n_bloc, 0) != -1){// Il existe une memoire libre
			faute++;
			int indice = exist(Liste_Bloc, n_bloc, 0); // Chercher ou se trouve la memoire libre
			Liste_Bloc[indice] = tache[i];      // Mettre la memoire libre avec le numero de la tache
			for(k=0;k<n_bloc;k++){
				printf(" %d ", Liste_Bloc[k]);
			}
			printf(" F\n");
			...
			if(iExist == 1){
				...
				int max_indice = 0;  // Indice dans le memoire qui est le plus loin tache ... Liste_Bloc
				int next_indice = 0; // Indice dans la liste de tache qui est le plus loin tache ...tache
				int* next_tache = (int *)malloc(n_bloc * sizeof(int));
				for(j=0;j<n_bloc;j++){
					next_tache[j] = exist(&(tache[i+1]), N_TACHE-i-1, Liste_Bloc[j]) + i + 1;
				}
				for(j=0;j<n_bloc;j++){
...
void FIFO(int * tache, int n_tache, int n_bloc){
  int faute = 0;
  int i, j, k, l;
  if(n_tache <= n_bloc){//quand le nombre de tache est moins que le nombre de bloc
    int * tab;
    ...
    printf("\nNombre de pages fautes : %d\n", faute);
		...
		// procédure de FIFO: quand le nombre de tache est plus que le nombre de bloc
		int * mem_bloc;
...
void LRU(int* tache, int n_tache, int n_bloc){
	...
	for(j=0;j<N_TACHE;j++){
		...
		//if already exist, if yes, refresh mp_n; if not, change  mp and refresh mp_n
		if(m >= 0){
			mp_n[m] = j;
			for(i=0;i<n_bloc;i++){
				printf(" %d ", mp[i]);
	...
	for(i=0;i<N_TACHE;i++){
		if(exist(Liste_Bloc,n_bloc,tache[i]) != -1){
			...
			while(Status_Bloc[*pMemoire] == 1){
				// Quand le status est 1 , mettre le status comme 0 et pMemoire move pour le suivant jusqu'à le status est 0
